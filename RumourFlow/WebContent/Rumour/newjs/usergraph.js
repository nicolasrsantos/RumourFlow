...
function createForceNetwork(type, nodes, edges) {
	...
	if (type == 2) {
		...
		//clExtent = d3.extent(d3.values(clustering._stringValues));
	} else {
		G = new jsnx.DiGraph();
		G.addNodesFrom(node_data);
		G.addEdgesFrom(edge_data);
    ...
    function colorByTweetType(edges) {
/*    	var values = [];
    	values = seq_color("#191919", "#bdbdbd", 4);
*/    	
    	d3.selectAll("path").transition().duration(1000).style("stroke",
			function(d, index) {
    			if (edges[index].type == "retweet")
					return "#bdbdbd";
				else if (edges[index].type == "quote")
    				return "#d9d9d9";
    			else if (edges[index].type == "reply")
					return "#000000";
		});
    }
    ...
    function colorByTimestamp(nodes) {
    	var values = [];
    	
    	var minTimestamp = Math.min.apply(Math, nodes.map(function(d){return d.timestamp;}));
    	var maxTimestamp = Math.max.apply(Math,nodes.map(function(d){return d.timestamp;}));
    	
    	var minDate = new Date(minTimestamp);
    	var maxDate = new Date(maxTimestamp);
    	
    	var diffInDays = daydiff(minDate, maxDate);
    	var belongsToInterval;
    	
    	// One color for each hour
    	if (diffInDays <= 30)
		{
    		values = seq_color("#662506", "#fff7bc", diffInDays + 1);
    		
    		d3.selectAll("circle").transition().duration(1000).style("fill",
				function(d, index) {
    				var currentDate = new Date(nodes[index].timestamp);
    				var currentDiff = daydiff(minDate, currentDate);
    				return values[currentDiff];
				}
    		);
		}
    	
    	// One color for each month (two years interval)
    	else if (diffInDays > 30 && diffInDays <= 730)
		{
    		values = seq_color("#662506", "#fff7bc", (diffInDays / 30) + 1);
		...
		{
    		values = seq_color("#662506", "#fff7bc", (diffInDays / 365) + 1);
    		
    		d3.selectAll("circle").transition().duration(1000).style("fill",
    				function(d, index) {
        				var currentDate = new Date(nodes[index].timestamp);
        				var currentDiff = daydiff(minDate, currentDate);
        				return values[currentDiff / 365];
    				}
        		);
		}
    }
	...
	function byClustering(nodes, edges) {
		...
		$.ajax({
			type : 'POST',
			data : JSON.stringify(clustering._stringValues),
			url : rootURL,
			contentType: "application/json",
			dataType: "text",
			success : function(data, err) {
	...
	function byBW() {
		...
		$.ajax({
			type : 'POST',
			data : JSON.stringify(betweenness._stringValues),
			url : rootURL,
			contentType: "application/json",
			dataType: "text",
			success : function(data, err) {
		...
		bwExtent = d3.extent(d3.values(betweenness));
